...
public:
    ...
    bool sporedi1 (const GolemBroj &b) const {
        if (this->broj.size()>b.broj.size()) { return true; }
        else if (this->broj.size()<b.broj.size()) { return false; }
        else {
            for (int i=this->broj.size()-1; i>-1; i--) {
...
struct compare {
    bool operator()(pair<GolemBroj, GolemBroj> a, pair<GolemBroj, GolemBroj> b) {
        return a.second.sporedi1(b.second);
    }
};

GolemBroj dijkstra (GolemBroj b) {
    GolemBroj i("3");
    GolemBroj prosti;
    priority_queue<pair<GolemBroj,GolemBroj>, vector<pair<GolemBroj, GolemBroj>>,compare>  pq;
    GolemBroj a("2");
    pq.push({a,a*a});
    prosti.dodadi(1);
    while (b.sporedi(i)==true) {
        if (pq.top().second.sporedi1(i)==true) {
            pq.push({i,i*i});
            prosti.dodadi(1);
        }
        else {
            while (pq.top().second.sporedi1(i)==false) {
                pair<GolemBroj, GolemBroj> tmp=pq.top();
